    ...
    <div className="space-y-6">
      ...
      >
        <SettingsRow label="Theme" description="Color theme preference">
          <SettingsSelect
            value={ui.theme}
            onChange={(value) => updateSettings("interface", { theme: value as typeof ui.theme })}
        ...
        <SettingsRow label="Accent Color" description="Primary accent color">
          <SettingsSelect
            ...
            onChange={(value) => updateSettings("interface", { accentColor: value as typeof ui.accentColor })}
        ...
        <SettingsRow label="Reduce Motion" description="Minimize animations">
          <SettingsSwitch
            ...
            onChange={(checked) => updateSettings("interface", { reduceMotion: checked })}
          />
        </SettingsRow>
      </SettingsSection>
      ...
      >
        <SettingsRow label="Font Size" description="Base font size in pixels">
          <SettingsSlider
            value={ui.fontSize}
            onChange={(value) => updateSettings("interface", { fontSize: value })}
            min={10}
            max={20}
            step={1}
          />
        ...
        <SettingsRow label="Interface Font" description="UI font family">
          <select
            value={ui.fontFamily}
            onChange={(e) => updateSettings("interface", { fontFamily: e.target.value })}
        ...
        <SettingsRow label="Code Font" description="Monospace font for code">
          <select
            ...
            onChange={(e) => updateSettings("interface", { codeFontFamily: e.target.value })}
            className="flex h-9 w-[180px] rounded-md border border-input bg-background px-3 py-1 text-sm shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring"
          >
        ...
        <SettingsRow label="Code Font Size" description="Font size for code blocks">
          <SettingsSlider
            value={ui.codeFontSize}
            onChange={(value) => updateSettings("interface", { codeFontSize: value })}
            min={10}
            max={18}
            step={1}
          />
      ...
      >
        <SettingsRow label="Sidebar Position" description="Position of the sidebar">
          <SettingsSelect
            value={ui.sidebarPosition}
            onChange={(value) => updateSettings("interface", { sidebarPosition: value as typeof ui.sidebarPosition })}
            options={SIDEBAR_POSITIONS}
          />
        </SettingsRow>

        <SettingsRow label="Sidebar Width" description="Width of the sidebar in pixels">
          <SettingsSlider
            value={ui.sidebarWidth}
            onChange={(value) => updateSettings("interface", { sidebarWidth: value })}
            min={200}
            max={400}
            step={10}
          />
        ...
        <SettingsRow label="Show Line Numbers" description="Display line numbers in editor">
          <SettingsSwitch
            checked={ui.showLineNumbers}
            onChange={(checked) => updateSettings("interface", { showLineNumbers: checked })}
          />
        </SettingsRow>
      </SettingsSection>
