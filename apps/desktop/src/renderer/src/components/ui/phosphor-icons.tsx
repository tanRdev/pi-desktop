/**
 * Phosphor icon surface for the desktop renderer.
 *
 * This file is kept (rather than importing directly from `@phosphor-icons/react`)
 * because it adds real value on top of the upstream package:
 *
 *   1. A custom `Pi` brand SVG icon that matches the Phosphor API shape.
 *   2. Standardized Tailwind size constants (`ICON_SIZE_*`) so every icon in the
 *      app picks from the same ladder of sizes instead of inventing one.
 *   3. A curated list of semantic aliases (e.g. `Settings`, `ChevronDown`,
 *      `Search`, `AlertTriangle`) that map generic product names onto the
