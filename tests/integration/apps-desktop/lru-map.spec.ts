...
import { LruMap } from "../../../apps/desktop/src/main/lru-map";
...
describe("LruMap", () => {
  ...
  it("refreshes recency on get so touched entries survive eviction", () => {
    ...
    // "b" should now be evicted as the oldest untouched entry.
  ...
  it("treats set on an existing key as a recency refresh", () => {
    ...
    // "b" should be evicted (now oldest), "a" retained with updated value.
    expect(cache.has("b")).toBe(false);
